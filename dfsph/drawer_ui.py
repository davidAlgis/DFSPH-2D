...
class UIDrawer:
    ...
    def __init__(
        ...
        self.play_icon = pygame.image.load(
            os.path.join(assets_dir, "play.png")
        ).convert_alpha()
        self.pause_icon = pygame.image.load(
            os.path.join(assets_dir, "pause.png")
        ).convert_alpha()
        self.step_icon = pygame.image.load(
            os.path.join(assets_dir, "step.png")
        ).convert_alpha()
        self.save_icon = pygame.image.load(
            os.path.join(assets_dir, "save.png")
        ).convert_alpha()

        # Scale the button background to fill the square.
        self.button_bg = pygame.transform.scale(
            self.button_bg, (button_size, button_size)
        )
        # Scale icons to 80% of the button size.
        icon_size = int(button_size * 0.8)
        self.play_icon = pygame.transform.scale(
            self.play_icon, (icon_size, icon_size)
        )
        self.pause_icon = pygame.transform.scale(
            self.pause_icon, (icon_size, icon_size)
        )
        self.step_icon = pygame.transform.scale(
            self.step_icon, (icon_size, icon_size)
        )
        self.save_icon = pygame.transform.scale(
            self.save_icon, (icon_size, icon_size)
        )
        ...
        self.pause_button = pygame.Rect(
            x0 + button_size + padding, y0, button_size, button_size
        )
        self.step_button = pygame.Rect(
            x0 + 2 * (button_size + padding), y0, button_size, button_size
        )
        self.save_button = pygame.Rect(
            x0 + 3 * (button_size + padding), y0, button_size, button_size
        )
    ...
    def draw_buttons(self, a):
        ...
        play_pos = (
            self.play_button.left + self.icon_offset[0],
        ...
        pause_pos = (
            self.pause_button.left + self.icon_offset[0],
        ...
        step_pos = (
            self.step_button.left + self.icon_offset[0],
        ...
        save_pos = (
            self.save_button.left + self.icon_offset[0],
