    ...
    for i in prange(n):
        ...
        for k in range(count):
            ...
            if is_solid[j] == 0:
                ...
                accum += water_viscosity * masses[j] * (v_dot_r/denom) * grad_ij
            else:
                denom = r2 * rho_i + 1e-5
                accum -= viscosity_coefficient_solid * masses[j] * (v_dot_r/denom) * grad_ij
...
def update_mass_solid_numba(positions, is_solid, neighbor_indices,
                            ...
                            gamma_mass_solid, masses_out, box):
    n = positions.shape[0]
    for i in prange(n):
        if not box.is_inside(positions[i, 0], positions[i, 1]):
            continue
...
def compute_pressure_forces_updated_numba(positions, is_solid, densities,
                                          ...
                                          neighbor_starts, neighbor_counts, h, box):
    n = positions.shape[0]
    ...
    for i in prange(n):
        ...
        for k in range(count):
            ...
            grad_ij = grad_w(positions[i], positions[j], h)
            if is_solid[j] == 0:
                accum -= (mass_i**2 * pressures[j]) / (rho_i * densities[j]) * grad_ij
            else:
...
def compute_surface_tension_forces_updated_numba(
        ...
        neighbor_starts, neighbor_counts, h, surface_tension_coeff, box):
    n = positions.shape[0]
    ...
    for i in prange(n):
        ...
        for k in range(count):
            ...
            if is_solid[j] == 0:
                grad_ij = grad_w(positions[i], positions[j], h)
                accum -= surface_tension_coeff * (masses[j] / (densities[j] + 1e-5)) * grad_ij
        surf_forces[i] = accum
    return surf_forces
